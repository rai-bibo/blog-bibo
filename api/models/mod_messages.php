<?php
// sample construct
class Mod_messages extends \RESTAPI\Model {
    ...
    public function get_messages($id = msisdn) {
        ...
        if ($id == '')
        return $this->query("select ".$this->fields." from users_api ORDER BY `username` LIMIT ".$this->start.", ".$this->limit);
        else
        return $this->query("select ".$this->fields." from users_api where id = '$id' and active=1 and deleted=0");
    }
}
